...
py::array_t<float> create_image(int h, int w);


py::array_t<float> create_image(int h, int w) {
    py::array_t<float> img({h, w, 3});
    auto buf = img.mutable_data();
...
py::array_t<float> run(int w, int h) {
    ...
    py::array_t<float> img_out = create_image(w, h);
    // get pointer to img_out
    float *img = (float *)img_out.request().ptr;
...
*/
            ...
            if (col[0] == std::numeric_limits<float>::infinity()) {
                ...
                // Color of the sphere.
                int index = (h - j - 1) * w * 3 + i * 3;
                img[index] = std::max(0.0f, std::min(col[0], 1.0f)); 
                img[index + 1] = std::max(0.0f, std::min(col[1], 1.0f)); 
                img[index + 2] = std::max(0.0f, std::min(col[2], 1.0f));
            }
...
PYBIND11_MODULE(raytracer, m) {
    ...
    m.def("run", &run, "A function which creates an image");
}